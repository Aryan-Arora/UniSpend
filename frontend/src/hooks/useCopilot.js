...
import { buildSystemPrompt, chatWithGroq } from '../services/groqService';

// ─────────────────────────────────────────────────────────────
// useCopilot
//
// AI financial co-pilot powered by Groq (llama-3.3-70b-versatile).
// Uses real financial data as context so the model can answer
// ANY financial question with personalized, data-backed answers.
// Falls back to local intent engine if Groq is unavailable.
// ─────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────
// Local Fallback (simplified — used if Groq is down)
// ─────────────────────────────────────────────────────────────
...
export const useCopilot = () => {
 ...
 // 2. Build conversation history for Groq (exclude greeting)
 const history = messages
 .filter((m) => m.id !== 'greeting')
 .map((m) => ({ role: m.role, text: m.text }));

 // 3. Try Groq first
 let response = await chatWithGroq(text.trim(), history, systemPrompt);

 // 4. Fall back to local engine if Groq fails
 if (!response) {
 console.log('[Copilot] Groq unavailable, using local fallback');
